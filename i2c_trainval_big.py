    ...
    while True:
        ...
        try:
            ...
            if c == 1:
                ...
                vectors["c%d" % (c)].append(data_list[0:4])
            r += 1
            if r > repeats:
                r = 1
                c += 1
...
def val_routine():
    ...
    def l2_distance(x, y):
        ...
        assert len(x) == 4
        result = 0.0
        result += (float(x[0])-float(y[0])) * (float(x[0])-float(y[0])) * 0.25
        result += (float(x[1])-float(y[1])) * (float(x[1])-float(y[1])) * 0.25
        result += (float(x[2])-float(y[2])) * (float(x[2])-float(y[2])) * 0.25
        result += (float(x[3])-float(y[3])) * (float(x[3])-float(y[3])) * 0.25
        return result

    print("start testing")

    # background class and reference vectors
    sum = 0
    for item in vectors["c1"]:
        sum += np.array(item)
    aver = sum / 3
    refer_vectors = {}
    for key in vectors.keys():
        if key != "c1":
            ...
            for item in vectors[key]:
                diff = np.array(item) - aver[0:4]
                refer_vectors[key].append(diff)
    ...
    while True:
        ...
        try:
            ...
            # start test
            print("test result:")
            diff_list = np.array(data_list) - aver
            res_list = []
            for ii in range(4):
                eval_list = []
                if ii < 3:
                    cut_list = diff_list[ii*2:ii*2+4]
                else:
                    cut_list = np.array((diff_list[0], diff_list[1], diff_list[6], diff_list[7]))
                l2 = l2_distance(cut_list, np.array((0, 0, 0, 0)))
                for _ in range(3):
                    eval_list.append([u"c1", l2])
                for key in refer_vectors.keys():
                    for item in refer_vectors[key]:
                        l2 = l2_distance(cut_list, item)
                ...
                for item in eval_list:
                    ...
                    if count[num] >= thresh:
                        res_list.append(num)
                        break
            if np.count_nonzero(np.array(res_list[0:3])) == 1:
                if res_list[0] != 0:
                    num = res_list[0]
                    print("the class is c%d, the position is left" % (num + 1))
                elif res_list[1] != 0:
                    num = res_list[1]
                    print("the class is c%d, the position is center" % (num + 1))
                else:
                    num = res_list[2]
                    print("the class is c%d, the position is right" % (num + 1))
            elif res_list[3] == 0 and res_list[1] != 0:
                num = res_list[1]
                ("the class is c%d, the position is center" % (num + 1))
            elif np.count_nonzero(np.array(res_list[0:3])) == 2:
                if res_list[0] == 0:
                    num = res_list[2]
                    print("the class is c%d, the position is right" % (num + 1))
                elif res_list[2] == 0:
                    ...
                    num = 0      
                    print("cannot judge position")
                    print("judge result: c%d, c%d, c%d" % (res_list[0] + 1, 
                                                        res_list[1] + 1,
                                                        res_list[2] + 1))
            elif np.count_nonzero(np.array(res_list[0:3])) == 0:
                num = 0      
                print("cannot judge position")
                print("judge result: c%d, c%d, c%d" % (res_list[0] + 1, 
                                                    res_list[1] + 1,
